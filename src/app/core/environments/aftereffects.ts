...
export default class Aftereffects extends General implements Host {
  ...
  import(itemPath, data, asSequence): void {
    ...
    if (fileFormat === ".aep") {
      this._jsxInjectorService.evalScript(
        `$._textAnimatorAfterEffects.importItem("${itemPath}", '${data}')`
      );
    } else if (fileFormat === ".ffx") {
      this._jsxInjectorService.evalScript(
        `$._textAnimatorAfterEffects.applyPreset("${itemPath}")`
      );
    } else if (fileFormat === ".mogrt") {
      this.extractMogrt(itemPath);
    } else {
      this._jsxInjectorService.evalScript(
        `$._textAnimatorAfterEffects.importFootage("${itemPath}", ${asSequence})`
      );
    }
  }
  ...
  openProject(itemPath): void {
    this._jsxInjectorService.evalScript(
      `$._textAnimatorAfterEffects.openAEPProject('${itemPath}')`
    );
  }
  setParameter(parameterIndex, parameterValue, key?): void {
    ...
    this._jsxInjectorService.evalScript(
      `$._textAnimatorAfterEffects.setLayerProperty(${parameterIndex}, ${parameterValue}, ${key})`
    );
  }
  setColorParameter(controllerIndex, { red, green, blue }, key?): void {
    this.setParameter(
      controllerIndex,
      this.convertColorBase255RelativeValueToBase1([red, green, blue, 255]),
      key
    );
  }
  settingHandler(settingData: any[]): any {
    const data = cloneDeep(settingData);
    const textsGroup = data.controllers.filter(
      text => text.name === "Text Inputs" && ControllerType.mainGroup
    );
    const otherGroups = data.controllers.filter(
      text => !(text.name === "Text Inputs" && ControllerType.mainGroup)
    );
  ...
  removeAllKeys(controllerIndex: number): void {
    this._jsxInjectorService.evalScript(
      `$._textAnimatorAfterEffects.removeAllKeys(${controllerIndex})`
    );
  }
  /**
  ...
  removeEffect(effectIndex: number): void {
    this._jsxInjectorService.evalScript(
      `$._textAnimatorAfterEffects.removeEffect('${effectIndex}')`
    );
  }
  ...
  openColorPicker(rgbColor): void {
    this._jsxInjectorService.evalScript(
      `$._textAnimatorAfterEffects.openColorPicker('${rgbColor[0]}','${
        rgbColor[1]
      }','${rgbColor[2]}',1)`
    );
  }
