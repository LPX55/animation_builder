...
export class SettingControllerService {
  constructor(
    private jsxInjectorService: JsxInjectorService,
  ...
  get groupedControllers(): any[] {
    ...
    controllers.forEach(value => {
      if (value.type === ControllerType.comment) {
        ...
        if (index > -1) {
          result[index].controllers.push(value);
        }
      }
    });
    return result;
  ...
  get nonGroupedControllers(): any[] {
    if (this.settingData.controllers) {
      const index = this.settingData.controllers.findIndex(
        item => item.type === ControllerType.comment
      );
      if (index < 0) {
        return this.settingData.controllers;
      }
  ...
  setParameter(
    ...
    return this.cepHostService.setParameter(
      parameterIndex,
      parameterValue,
      key,
      controller,
  ...
  setColorParameter(controllerIndex, { red, green, blue }): void {
    return this.cepHostService.setColorParameter(controllerIndex, {
      red,
      green,
      blue
    });
  ...
  changeRTLStatus(groupIndex: number): void {
    return this.jsxInjectorService.evalScript(
      `$._textAnimatorAfterEffects.changeRTLStatus(${groupIndex})`
    );
  }
  locationBack(): void {
    this.location.back();
